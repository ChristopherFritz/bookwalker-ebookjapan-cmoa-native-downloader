...
- **~2× faster descrambling**. Tiles are drawn straight onto the output canvas instead of being round-tripped through `getImageData`/`putImageData`. Output is byte-identical to the previous descrambler across six tile geometries.
- **Choose the page image format**. JPEG (default, q0.92), WebP, lossless, or PNG, under **Image format** in the panel. The CDN already serves a lossy JPEG, so this is a second generation; lossless is worth it mainly on flat line art, which the panel notes.
