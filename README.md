...
- **Save as ZIP**. Fetches every page of the open book (manga, light novels, and trial/free samples), descrambles the tile-shuffled images, and saves a ZIP as `Series/Volume 2/page-0001.jpg`, `page-0002.jpg`, … (rename it `.cbz` for CBZ-capable readers).
- **Save and run through Mokuro**. Sends the pages to [mokuro-bridge](https://github.com/GolyBidoof/mokuro-bridge), which runs [GolyBidoof/mokuro](https://github.com/GolyBidoof/mokuro) (a fork of [kha-white/mokuro](https://github.com/kha-white/mokuro)) Japanese OCR and produces the `.cbz` / `.mokuro` / `.webp` trio for [reader.mokuro.app](https://reader.mokuro.app/).
...
- **Every socket it can find**. Chrome allows only 6 concurrent HTTP/1.1 connections per *origin*, and the page CDN is a single HTTP/1.1 host, so a page lane alone is pinned at 6. The script spreads requests across every origin available to it — the page, Tampermonkey's background context, a trailing-dot host, and each fetch-proxy port mokuro-bridge exposes — measured at **294 concurrent sockets** with the bridge up, against 6 without it.
- **~2× faster descrambling**. Tiles are drawn straight onto the output canvas instead of being round-tripped through `getImageData`/`putImageData`; output is byte-identical (verified against the reference descrambler across six tile geometries).
- **Choose the page image format**. JPEG (default, q0.92), WebP, lossless, or PNG, under **Image format** in the panel. The CDN already serves a lossy JPEG, so this is a second generation — lossless is only worth it on flat line art, and the panel says so.
- **A panel that gets out of your way**. Draggable, resizable, minimizable, with ARIA roles; remembers its size and position; never steals your arrow keys (Left/Right keep flipping viewer pages); a **»** button flaps the whole panel off-screen to a **«** tab on the right edge.

## Why this one is different

Most scripts for this site are *page-turners* that flip through the book and capture what renders, or *canvas scrapers* that screenshot the viewer page by page. This script is neither:

| | Typical page-turn / canvas-capture scripts | **This script** |
| --- | --- | --- |
| How pages are obtained | Flip through the viewer, wait for render, capture | **Fetch the CDN files directly** with the viewer's own signed URLs |
| Output quality | Whatever the viewer drew on screen | **Original page resolution**, unscrambled offline, cropped to the declared size |
| Speed | Serial, one page at a time, bound to render speed | Parallel prefetch + a **worker-pool descramble engine** |
| Expiring links | One stale URL and the run dies | **Auto-renews the CloudFront policy mid-run**, with 403/429 breakers, cooldowns, and retry rounds |
...
**It doesn't fight your reading.** Arrow keys stay with the viewer, the panel starts compact and only widens when there is something to show, it makes no background requests while idle, and after a finished run it clears its cache so nothing accumulates in your browser's storage.
...
**Since v1.5.0 this script also uses the bridge as a socket multiplier.** Chrome's 6-connections-per-origin cap is keyed on scheme + host + **port**, so a local port is a fresh origin. The bridge opens a range of fetch-proxy ports (48 by default) and advertises them on `/health` as `fetchProxyPorts`; the script turns each one into a lane and routes page fetches through the least-loaded of them. The bridge raises its file-descriptor limit to hold that many listeners. Nothing is required from you — start the bridge and the extra sockets are there; the panel's **?** next to the bridge row shows the live count. Its [CORS allow-list defaults to the four BookWalker web viewers](https://github.com/GolyBidoof/mokuro-bridge), which is what lets this userscript POST pages from the browser. Per volume it runs:
...
3. **Chunked OCR**. Pages are OCR'd **as they arrive** in chunks (default 8 pages, 1.5 s idle flush), so capture and OCR overlap. The fork's batched API streams per-page progress back.
...
The engine does the actual reading, in an optimized fork of [kha-white/mokuro](https://github.com/kha-white/mokuro):

- **Batched OCR inference**. Upstream runs one model `generate()` call **per text line**; the fork batches all text-line crops from a chunk into **one call per `ocr_batch_size` crops** (64 on Apple Silicon, 32 on CUDA, 16 on CPU).
- **Concurrent page loading**. Pages are decoded on a thread pool instead of sequentially.
- **Hardware-aware defaults**. `mokuro/config.py` auto-detects your machine: Apple Silicon gets 8 workers / batch 64 / fp16; NVIDIA gets 4 workers / batch 32 / fp16 + conv+bn fusion + `torch.compile`; CPU gets cores/2 workers / batch 16. All tunable from one `EDIT ME` block.
...
- **[GolyBidoof / mokuro](https://github.com/GolyBidoof/mokuro)**, this project's OCR engine: a fork of kha-white/mokuro, used by mokuro-bridge.
- **[kha-white / mokuro](https://github.com/kha-white/mokuro)**, the upstream OCR engine this fork is based on.
